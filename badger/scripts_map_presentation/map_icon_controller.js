...
const displayVillageStateIcon = () => {
  ...
  switch (get(MAP_KEY.STATE)) {
    case MAP_ICON_STATE.UNHURT:
      // If the village is unhurt and has no recap information (see `displayInvasionRecapIcon`) or not targeted for an attack, then
      // display a peaceful note
      if (!recapIsShown && !exists(MAP_KEY.INVASION_INTENTION)) {
        makeEntity(
          ICON_GROUP.VO_TOOLTIP,
          MAP_EXTENDED_VO_TOOLTIP.INVASION_VILLAGE_PEACEFUL
        );
      }
      break;
    case MAP_ICON_STATE.DAMAGED:
